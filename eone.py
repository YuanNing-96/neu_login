...
def auth_eone_login(username, password):
    ...
    try:
        ...
        return 601
    if "锁定".encode("utf-8") in res.content:
        # 连续登录失败5次，账号将被锁定1分钟，剩余次数3
        return 401
    if "访问被拒绝".encode("utf-8") in res.content:
        # 访问被拒绝
        return 402
    doc = pq(res.content)
    ...
    # 受页面改版影响，该方法可能不稳定
    if eone_account.encode("utf-8") in res.content:
        return 200
    elif "5分钟" in res:
...
def auth_eone_logout():
    ...
    if "失败".encode("utf-8") in res.content:
        return False
    if "下线请求已发送".encode("utf-8") in res.content:
        return True
    return False
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        print(online_info.__class__)
        print(online_info[0])
        if online_info[0] == "b'not_online'":
            # 本机未在线，进行登入程序
